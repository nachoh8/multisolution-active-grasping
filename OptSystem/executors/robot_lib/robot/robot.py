...
from .trust_region import TrustRegionState, update_state, generate_batch
from .gp_utils.update_models import update_surr_model
from .gp_utils.ppgpr import GPModelDKL
...
class RobotState:
    ...
    def asymmetric_acquisition(self, max_calls):   
        ...
        for state in self.rank_ordered_trs:
            ...
            if self.objective.num_calls >= max_calls:
                continue

            if len(feasible_searchspace_pts) + self.objective.num_calls > max_calls:
                n = max_calls - self.objective.num_calls
                feasible_searchspace_pts = feasible_searchspace_pts[:n]
